...
IMAGE_NAME="python-djongo-app"
BACKUP_IMAGE_NAME="${IMAGE_NAME}:backup"
CONTAINER_NAME="djongo-container"
CONTAINER_EXPOSE_PORT=4000  # Django's default port
...
if [ $? -eq 0 ]; then
    ...
    docker run -d -p "$CONTAINER_EXPOSE_PORT":8000 --name "$CONTAINER_NAME" "$IMAGE_NAME"
